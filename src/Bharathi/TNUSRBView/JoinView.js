...
const JoinView = () => {
  ...
  const handleDelete = async (id) => {
    ...
    if (result.isConfirmed) {
      setLoading(true); // Start loading
      try {
        await axios.delete(`https://www.bharathithervukalam.com/api/join/delete/${id}`);
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      const response = await axios.put(`https://www.bharathithervukalam.com/api/join/update/${currentGroup.id}`, updatedFormData, {
        headers: {
  ...
  return (
    <div className="container">
      <h2 className='text-center'>Joint Recruitment for Various Posts Syllabus & Notes </h2>
