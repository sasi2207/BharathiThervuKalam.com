...
const GroupIVView = () => {
  ...
  const handleDelete = async (id) => {
    ...
    if (result.isConfirmed) {
      try {
        await axios.delete(`https://www.bharathithervukalam.com/api/group4/delete/${id}`);
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      const response = await axios.put(`https://www.bharathithervukalam.com/api/group4/update/${currentGroup.id}`, updatedFormData, {
        headers: {
  ...
  return (
    <div className="container">
      <h2 className='text-center'>GROUP IV SYLLABUS & NOTES</h2>
