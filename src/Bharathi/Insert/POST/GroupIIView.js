...
const GroupIIView = () => {
  ...
  const handleDelete = async (id) => {
    ...
    if (result.isConfirmed) {
      try {
        await axios.delete(`https://www.bharathithervukalam.com/api/group2/delete/${id}`);
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      const response = await axios.put(`https://www.bharathithervukalam.com/api/group2/update/${currentGroup.id}`, updatedFormData, {
        headers: {
  ...
  return (
    <div className="container">
      <h2 className='text-center'>GROUP II SYLLABUS & NOTES</h2>
